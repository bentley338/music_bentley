document.addEventListener('DOMContentLoaded', () => {
    ...
    let playlist = [
        ...
        {
            title: "Iris",
            artist: "Goo Goo Dolls", // Perbaikan typo
            src: "iris.mp3",
        ...
        },
        {
            ...
            artist: "Taylor Swift",
            src: "message_in_a_bottle.mp3", // **FIXED TYPO HERE**
        ...
        },
        {
            ...
            info: `<b>🎶 Lesung Pipi – Raim Laode</b><br><br>
                Lagu ini adalah ungkapan kekaguman terhadap seseorang yang memiliki lesung pipi, sebuah detail kecil yang membuatnya semakin istimewa. Raim Laode menggambarkan bagaimana lesung pipi menjadi daya tarik yang manis dan memikat, membuat hati terpikat setiap kali melihat senyuman. Lagu ini sederhana namun penuh pesona, merayakan keindahan dalam detail-detail kecil yang membuat seseorang menjadi unik dan tak terlupakan.`
        }
    ];
    ...
    function loadSong(songIndex) {
        ...
        audioPlayer.src = song.src;
        audioPlayer.load(); // Memuat sumber audio
        currentAlbumArt.src = song.albumArt;
        currentSongTitle.textContent = song.title;
        currentArtistName.textContent = song.artist;
        infoText.innerHTML = song.info;
    ...
    function attemptPlay() {
        ...
        // Pastikan lagu sudah dimuat sebelum mencoba play
        if (!audioPlayer.src || audioPlayer.src === window.location.href) {
            console.warn("Audio source not loaded or invalid. Attempting to load first song.");
            loadSong(0); // Coba muat lagu pertama jika belum ada
        }
    ...
    audioPlayer.addEventListener('loadedmetadata', () => {
        if (!isNaN(audioPlayer.duration)) {
            ...
            // Pastikan gainNode dan effectLevelGainNode diatur ulang setiap kali metadata dimuat
            gainNode.gain.setValueAtTime(masterVolumeControl.value, audioContext.currentTime);
            ...
            // Jika statusnya "playing" sebelum metadata dimuat, coba putar lagi
            // Ini membantu jika ada jeda karena buffering atau perubahan sumber
            if (isPlaying && audioPlayer.paused) {
                audioPlayer.play().catch(e => console.warn("Failed to auto-play after loadedmetadata:", e));
            }
    ...
    function restoreOriginalPlaylist() {
        playlist = [
            ...
            },
            {
                ...
                albumArt: "album_art_message_in_a_bottle.jpg",
                info: `<b>🎶 Message In A Bottle – Taylor Swift</b><br><br>
                    Terinspirasi dari perasaan kerinduan dan harapan, "Message In A Bottle" adalah lagu yang menggambarkan upaya untuk menjaga kenangan dan kenangan cinta tetap hidup, meskipun terpisah jarak. Seperti pesan dalam botol yang dilemparkan ke lautan, lagu ini adalah doa agar suatu hari pesan cinta dan kenangan akan menemukan jalannya kembali kepada orang yang dituju. Melodinya yang menyentuh hati mencerminkan emosi mendalam tentang kehilangan dan harapan.`
            },
            {
                title: "Supernatural",
                artist: "Ariana Grande",
        ...
        function restoreOriginalPlaylist() {
            // Restore from the original `originalPlaylistOrder` if it's set
            if (originalPlaylistOrder.length > 0) {
                playlist = [...originalPlaylistOrder];
            } else {
                // Fallback to initial hardcoded list if originalPlaylistOrder is empty
                playlist = [
                    {
                        title: "Guilty as Sin",
                        artist: "Taylor Swift",
            ...
            // Setelah playlist dikembalikan, pastikan currentSongIndex valid
            const currentSongSource = audioPlayer.src.split('/').pop();
            if (currentSongSource) {
                const newIndex = playlist.findIndex(song => song.src === currentSongSource);
                if (newIndex !== -1) {
